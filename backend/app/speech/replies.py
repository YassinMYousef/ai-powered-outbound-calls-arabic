...
"""
from app.speech.greeting import GreetingContext, follow_up_question, greeting_text
...
def static_phrases() -> list[str]:
    """Phrases whose wording never varies per call — safe to pre-synthesize once.

    These are the closing/transfer/offer lines plus the generic (no name, ticket
    or procedure) greeting and repeat-question. Pre-generating their audio to
    disk (see telephony/audio_store.prewarm_disk_cache) means playback never
    blocks on a live TTS round-trip, so the caller hears no dead air before the
    greeting or the closing message.
    """
    generic = GreetingContext()
    return [
        greeting_text(generic),
        repeat_question_text(generic),
        OFFER_HELP_AR,
        TRANSFER_AR,
        TRANSFER_UNAVAILABLE_AR,
        RESOLVED_GOODBYE_AR,
